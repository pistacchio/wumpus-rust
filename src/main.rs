...
impl Maze {
    ...
    fn describe_room(&self, room: usize) -> String {
        ...
        if let Some(_) = self.is_wumpus_nearby(room) {
            desc_lines.push("You smell something terrible nearby.".to_string());
        }

        let neighbors: Vec<String> = self.rooms[room].neighbors
            .iter()
            .map(|i| i.to_string())
            .collect();
    ...
    fn is_danger_nearby(&self, room: usize, danger: RoomContents) -> bool {
        self.rooms[room].neighbors
            .iter()
    ...
    fn parse_room(&self, destination: &str, current_room: usize) -> Result<usize, String> {
        let dest: usize = destination
            .trim()
            .parse()
            .map_err(|e: std::num::ParseIntError| e.description().to_string())?;

        // Check that the given destination is the id of a linked room.
        if self.rooms[current_room].neighbors.iter().all(|&r| r != dest) {
...
fn main() {
    ...
    loop {
        ...
        match status {
            ...
            Moving => match maze.parse_room(input, player.room) {
                Ok(room) => {
                    if maze.rooms[room].wumpus {
                        ...
                        player.room = maze.rng.gen_range(0, maze.rooms.len());
                    } else {
                        player.room = room;
                    }
            ...
            Shooting => match maze.parse_room(input, player.room) {
                Ok(room) =>  {
                    if maze.rooms[room].wumpus {
                    ...
                    if let Some(wumpus_room) = maze.is_wumpus_nearby(room) {
                        // 75% chances of waking up the wumpus that would go into another room
                        if maze.rng.gen::<f32>() <= WAKE_WUMPUS_PROB {
                            let new_wumpus_room =
                                *maze.rooms[wumpus_room].neighbors.choose(&mut maze.rng)
                                .unwrap();
                            ...
                            maze.rooms[new_wumpus_room].wumpus = true;
